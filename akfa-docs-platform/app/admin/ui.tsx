...
import { Copy, Eye, ImagePlus, LogOut, Plus, RefreshCw, Save, ShieldCheck, ShieldOff, Trash2, Upload } from 'lucide-react';
...
export function AdminDashboard({ adminEmail, csrf }: { adminEmail: string; csrf: string }) {
  ...
  const [downloadRules, setDownloadRules] = useState<{ maxMb: number; allowedExtensions: string[] }>({ maxMb: 200, allowedExtensions: [] });
  ...
  async function loadDownloads() {
    const response = await api<DownloadsResponse>('/admin/api/downloads', csrf);
    setDownloads(response.downloads);
    setDownloadRules({ maxMb: response.maxMb, allowedExtensions: response.allowedExtensions });
  }
  ...
  async function replaceDownload(key: string, event: React.ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    if (!file) return;
    const body = new FormData();
    body.append('key', key);
    ...
    setError('');
    try {
      await api('/admin/api/downloads', csrf, { method: 'POST', body });
      await loadDownloads();
      setMessage('Файл заменён');
    } catch (err) {
  ...
  async function copyDownloadLink(path: string) {
      ...
      <div className="admin-wrap">
        ...
        <div className="admin-grid" style={{ marginTop: 16 }}>
          ...
          <main className="admin-card">
            ...
            <section className="security-panel">
              <div>
                <h2 style={{ margin: 0 }}>Файлы / Загрузки</h2>
                <p style={{ color: 'var(--muted)', marginTop: 6, marginBottom: 0 }}>
                  Стабильные публичные ссылки для APK, EXE, DMG и других клиентских файлов.
                </p>
              </div>
              <div className="twofa-box" style={{ width: '100%' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}>
                  <span className="status-pill">До {downloadRules.maxMb} MB · {downloadRules.allowedExtensions.join(', ')}</span>
                  <button className="btn secondary" type="button" onClick={() => loadDownloads().catch((err) => setError(err instanceof Error ? err.message : 'Файлы недоступны'))}>
                    ...
                    <div className="download-row" key={item.key}>
                      <div>
                        ...
                        <span style={{ color: 'var(--muted)' }}>
                          {item.filename} · {item.exists ? `${item.size ? Math.round(item.size / 1024 / 1024 * 10) / 10 : 0} MB` : 'нет файла'}
                          {item.updatedAt ? ` · ${new Date(item.updatedAt).toLocaleString('ru-RU')}` : ''}
                        </span>
                        <br />
                        <code>{item.publicPath}</code>
                      </div>
                      <div className="button-row" style={{ justifyContent: 'flex-start' }}>
                        <label className="btn" style={{ cursor: 'pointer' }}>
                          <Upload size={16} /> {downloadBusy === item.key ? 'Загрузка...' : 'Заменить'}
                          <input
                            hidden
                            type="file"
                            accept={downloadRules.allowedExtensions.map((item) => `.${item}`).join(',')}
                            onChange={(event) => replaceDownload(item.key, event)}
                            disabled={downloadBusy === item.key}
                          />
                        </label>
                        <button className="btn secondary" type="button" onClick={() => copyDownloadLink(item.publicPath)}>
                          <Copy size={16} /> Скопировать
                        </button>
                      </div>
                    </div>
                  ))}
                  {!downloads.length ? <p style={{ color: 'var(--muted)' }}>Manifest downloads не найден.</p> : null}
                </div>
              </div>
            </section>
