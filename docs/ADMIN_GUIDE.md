...
10. Откройте карточку пользователя.
11. Скопируйте готовое сообщение для пользователя.
12. Передайте сообщение пользователю.

### Как выдать пользователю ссылку подключения

Для одного пользователя:

1. Откройте `https://PANEL_DOMAIN`.
2. Перейдите в раздел **Пользователи VPN**.
3. Откройте карточку нужного пользователя.
4. Найдите блок **Сообщение для пользователя**.
5. Нажмите **Скопировать сообщение**.
6. Отправьте текст пользователю в Telegram, WhatsApp или email.

В сообщении уже есть обращение к пользователю, connect-ссылка и короткая инструкция. Пользователю нужно открыть ссылку и следовать инструкциям на странице подключения.

Для массовой выдачи:

1. Откройте раздел **Пользователи VPN**.
2. Нажмите **Скачать сообщения**.
3. Откройте Excel-файл.
4. Используйте колонку **Готовое сообщение** для отправки пользователям.

В таблице одна строка соответствует одному пользователю. Есть фамилия, имя, логин, статус, отдел, профиль доступа, connect-ссылка и готовое сообщение для каждого пользователя. Колонка **Готовое сообщение** содержит полный текст для отправки, включая переносы строк внутри одной ячейки.
...
На dashboard главная traffic-метрика — общий трафик VPS:

- `Общий сетевой трафик VPS` — весь трафик сетевого интерфейса сервера: VPN, Nginx, downloads, SSH, certbot, обновления и другие процессы.
- `VPN-трафик через Xray` показывается отдельно и нужен для диагностики VPN-нагрузки.

Раздел `Аналитика трафика` — это VPN-трафик пользователей из Xray stats. Именно его используйте для анализа конкретного человека, устройства, ноды и лимитов.
...
## 17. Название и подсказки в VPN-клиентах

В админке откройте:

```text
Настройки администратора -> Подписка в VPN-клиентах
```

Можно настроить:

- название подписки, которое уходит в `profile-title`;
- имя файла для Clash/FClashX через `Content-Disposition`;
- текст подсказки для Clash YAML comments;
- интервал автообновления через `profile-update-interval`;
- prefix серверов, например `AKFA` в `AKFA 🇳🇱 Нидерланды`.

Backend также отдаёт `subscription-userinfo` с трафиком и сроком действия, если они есть у пользователя. Это помогает Happ/FClashX и совместимым клиентам показывать лимит, расход и дату истечения.

Для Happ backend добавляет в начало raw-подписки служебные строки `#profile-title`, `#subscription-userinfo`, `#profile-update-interval`, `#profile-web-page-url` и `#announce`. Это официальный metadata-формат Happ для названия и объявления. Для остальных raw-клиентов тело остаётся списком чистых `vless://` строк.

## 18. Что нельзя делать без бэкапа
